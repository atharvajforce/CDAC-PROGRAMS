...
public class Array2DService {
	...
	public static int[][] rotateColumnLeft(int[][] arr, int n) {
	    int rows = arr.length;
	    int cols = arr[0].length;
	    int[][] temp = new int[rows][cols];

	    // Copy first column into temp1
	    int[] temp1 = new int[rows];
	    for (int i = 0; i < rows; i++) {
	        temp1[i] = arr[i][0];
	    }

	    // Shift all columns to the left by 1
	    for (int i = 0; i < rows; i++) {
	    ...
	    // Place the original first column into the last column
	    for (int i = 0; i < rows; i++) {
	        temp[i][cols - 1] = temp1[i];
	    }
